//
//  EditMonthlyTransactionView.swift
//  Expenso
//
//  Created by Hendrik Steen on 03.06.22.
//

import SwiftUI

struct EditMonthlyTransactionView: View {
                            ...
                            content: {
                                Alert(title: Text(APP_NAME), message: Text("Are you sure you want to delete this transaction?"),
                                    primaryButton: .destructive(Text("Delete")) {
                                        viewModel.deleteMonthlyTransaction(managedObjectContext: self.managedObjectContext)
                ...
                VStack {
                    ...
                    VStack {
                        Button(action: { viewModel.saveMonthlyTransaction(managedObjectContext: managedObjectContext) }, label: {
                            HStack {
                                Spacer()
                                TextView(text: "\(viewModel.getButtText()) FOR EVERY MONTH", type: .button).foregroundColor(.white)
                                Spacer()
                            }
                        })
